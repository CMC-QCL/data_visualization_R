...
date: "Wednesday, February 10, 2023"
output:
...
This component maps the variables in our data onto the visual properties of our graph, such as the x and y coordinates. In order to use this component, call the `aes()` function inside `ggplot()`. For example, to plot the average IMDB rating (`averageRating`) against the release year of the title (`startYear`)
...
Create a scatter plot of the budget vs. the revenue
...
Now try the other aesthetic attributes: shape and size. First, make a base plot without any aesthetic arguments.
...
As an example, let's change add a title to the scatter plot of budget vs. revenue colored by platform.
